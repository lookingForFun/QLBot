...
template<class Ti> __forceinline Ti	Max( Ti x, Ti y ) { return ( x > y ) ? x : y; }
/*
...
// RAVEN END
*/
//template<class Ti> __forceinline Ti	Square( Ti x ) { return x * x; }
template<class Ti> __forceinline Ti	Max3( Ti x, Ti y, Ti z ) { return ( x > y ) ? ( ( x > z ) ? x : z ) : ( ( y > z ) ? y : z ); }
template<class Ti> __forceinline Ti	Min3( Ti x, Ti y, Ti z ) { return ( x < y ) ? ( ( x < z ) ? x : z ) : ( ( y < z ) ? y : z ); }
template<class Ti> __forceinline int	Max3Index( Ti x, Ti y, Ti z ) { return ( x > y ) ? ( ( x > z ) ? 0 : 2 ) : ( ( y > z ) ? 1 : 2 ); }
template<class Ti> __forceinline int	MaxIndex( Ti x, Ti y ) { return  ( x > y ) ? 0 : 1; }
template<class Ti> __forceinline int	MinIndex( Ti x, Ti y ) { return ( x < y ) ? 0 : 1; }
template<class Ti> __forceinline Ti	Min( Ti x, Ti y ) { return ( x < y ) ? x : y; }
template<class Ti> __forceinline int	Min3Index( Ti x, Ti y, Ti z ) { return ( x < y ) ? ( ( x < z ) ? 0 : 2 ) : ( ( y < z ) ? 1 : 2 ); }
template<class Ti> __forceinline Ti	Sign( Ti f ) { return ( f > 0 ) ? 1 : ( ( f < 0 ) ? -1 : 0 ); }
template<class Ti> __forceinline Ti	SignZero( Ti f ) { return ( f > 0 ) ? 1 : ( ( f < 0 ) ? -1 : 1 ); }
template<class Ti> __forceinline Ti	Cube( Ti x ) { return x * x * x; }
...
public:
...
#endif
	static const int idMath::INT_MIN			= (-2147483647 - 1);
	static const int idMath::INT_MAX			= 2147483647;
	...
	static unsigned char		Ftob( float f );			// float to byte conversion, the result is clamped to the range [0-255]

	static signed char			ClampChar( int i );
// RAVEN BEGIN
	static unsigned char		ClampByte( int i );
// RAVEN END
...
private:
	...
	static unsigned long		iSqrt[SQRT_TABLE_SIZE];
...
__forceinline float idMath::RSqrt( float x ) {
	long i;
	float y, r;
...
#ifdef _FAST_MATH
__forceinline float idMath::InvSqrt16( float x ) {
...
__forceinline float idMath::InvSqrt( float x ) {
	dword a = ((union _flint*)(&x))->i;
	union _flint seed;
...
__forceinline double idMath::InvSqrt64( float x ) {
	dword a = ((union _flint*)(&x))->i;
	union _flint seed;
...
__forceinline float idMath::Sqrt16( float x ) {
	return x * InvSqrt16( x );
}

__forceinline float idMath::Sqrt( float x ) {
	return x * InvSqrt( x );
}

__forceinline double idMath::Sqrt64( float x ) {
	return x * InvSqrt64( x );
}

__forceinline float idMath::Sin( float a ) {
	return sinf( a );
}

__forceinline float idMath::Sin16( float a ) {
	float s;
...
__forceinline double idMath::Sin64( float a ) {
	return sin( a );
}

__forceinline float idMath::Cos( float a ) {
	return cosf( a );
}

__forceinline float idMath::Cos16( float a ) {
	float s, d;
...
__forceinline double idMath::Cos64( float a ) {
	return cos( a );
}
#endif

__forceinline void idMath::SinCos( float a, float &s, float &c ) {
...
__forceinline void idMath::SinCos16( float a, float &s, float &c ) {
	float t, d;
...
__forceinline void idMath::SinCos64( float a, double &s, double &c ) {
#ifdef ID_WIN_X86_ASM
	_asm {
		fld		a
		fsincos
...
#ifdef _FAST_MATH
__forceinline float idMath::Tan( float a ) {
	return tanf( a );
}

__forceinline float idMath::Tan16( float a ) {
	float s;
	bool reciprocal;
...
__forceinline double idMath::Tan64( float a ) {
	return tan( a );
}
#endif

__forceinline float idMath::ASin( float a ) {
	if ( a <= -1.0f ) {
		return -HALF_PI;
	}
	if ( a >= 1.0f ) {
...
__forceinline float idMath::ASin16( float a ) {
...
__forceinline double idMath::ASin64( float a ) {
	if ( a <= -1.0f ) {
		return -HALF_PI;
	}
	if ( a >= 1.0f ) {
...
__forceinline float idMath::ACos( float a ) {
	if ( a <= -1.0f ) {
		return PI;
	}
	if ( a >= 1.0f ) {
...
__forceinline float idMath::ACos16( float a ) {
	if ( FLOATSIGNBITSET( a ) ) {
		if ( a <= -1.0f ) {
			return PI;
		}
...
__forceinline double idMath::ACos64( float a ) {
	if ( a <= -1.0f ) {
		return PI;
	}
	if ( a >= 1.0f ) {
...
#ifdef _FAST_MATH
__forceinline float idMath::ATan( float a ) {
	return atanf( a );
}

__forceinline float idMath::ATan16( float a ) {
	float s;
...
__forceinline double idMath::ATan64( float a ) {
	return atan( a );
}

__forceinline float idMath::ATan( float y, float x ) {
	return atan2f( y, x );
}

__forceinline float idMath::ATan16( float y, float x ) {
	float a, s;
...
__forceinline double idMath::ATan64( float y, float x ) {
	return atan2( y, x );
}
#endif

__forceinline float idMath::Pow( float x, float y ) {
	return powf( x, y );
}

__forceinline float idMath::Pow16( float x, float y ) {
	return Exp16( y * Log16( x ) );
}

__forceinline double idMath::Pow64( float x, float y ) {
	return pow( x, y );
}

__forceinline float idMath::Exp( float f ) {
	return expf( f );
}

__forceinline float idMath::Exp16( float f ) {
...
__forceinline double idMath::Exp64( float f ) {
	return exp( f );
}

__forceinline float idMath::Log( float f ) {
	return logf( f );
}

__forceinline float idMath::Log16( float f ) {
	int i, exponent;
	float y, y2;
...
__forceinline double idMath::Log64( float f ) {
	return log( f );
}

__forceinline int idMath::IPow( int x, int y ) {
	int r; for( r = x; y > 1; y-- ) { r *= x; } return r;
}

__forceinline int idMath::ILog2( float f ) {
	return ( ( ( *reinterpret_cast<int *>( &f ) ) >> IEEE_FLT_MANTISSA_BITS ) & ( ( 1 << IEEE_FLT_EXPONENT_BITS ) - 1 ) ) - IEEE_FLT_EXPONENT_BIAS;
}

__forceinline int idMath::ILog2( int i ) {
	return ILog2( (float)i );
}

__forceinline int idMath::BitsForFloat( float f ) {
	return ILog2( f ) + 1;
}

__forceinline int idMath::BitsForInteger( int i ) {
	return ILog2( (float)i ) + 1;
}

__forceinline int idMath::MaskForFloatSign( float f ) {
	return ( ( *reinterpret_cast<int *>( &f ) ) >> 31 );
}

__forceinline int idMath::MaskForIntegerSign( int i ) {
	return ( i >> 31 );
}

__forceinline int idMath::FloorPowerOfTwo( int x ) {
	return CeilPowerOfTwo( x ) >> 1;
}

__forceinline int idMath::CeilPowerOfTwo( int x ) {
	x--;
	x |= x >> 1;
	x |= x >> 2;
	x |= x >> 4;
...
__forceinline bool idMath::IsPowerOfTwo( int x ) {
	return ( x & ( x - 1 ) ) == 0 && x > 0;
}

__forceinline int idMath::BitCount( int x ) {
	x -= ( ( x >> 1 ) & 0x55555555 );
...
__forceinline int idMath::BitReverse( int x ) {
...
__forceinline int idMath::Abs( int x ) {
   int y = x >> 31;
   return ( ( x ^ y ) - y );
}

__forceinline float idMath::Fabs( float f ) {
	int tmp = *reinterpret_cast<int *>( &f );
	tmp &= 0x7FFFFFFF;
	return *reinterpret_cast<float *>( &tmp );
}

__forceinline float idMath::Floor( float f ) {
	return floorf( f );
}

__forceinline float idMath::Ceil( float f ) {
	return ceilf( f );
}

__forceinline float idMath::Rint( float f ) {
	return floorf( f + 0.5f );
}

__forceinline int idMath::Ftoi( float f ) {
#ifdef ID_WIN_X86_SSE
...
__forceinline int idMath::FtoiFast( float f ) {
#ifdef ID_WIN_X86_ASM
	int i;
	__asm fld		f
...
__forceinline unsigned char idMath::Ftob( float f ) {
#ifdef ID_WIN_X86_SSE
...
__forceinline signed char idMath::ClampChar( int i ) {
	if ( i < -128 ) {
		return -128;
	}
	if ( i > 127 ) {
...
// RAVEN BEGIN
__forceinline unsigned char idMath::ClampByte( int i ) 
{
	if( i < 0 ) 
	{
		return( 0 );
...
__forceinline signed short idMath::ClampShort( int i ) {
	if ( i < -32768 ) {
		return -32768;
	}
	if ( i > 32767 ) {
...
__forceinline int idMath::ClampInt( int min, int max, int value ) {
	if ( value < min ) {
		return min;
	}
	if ( value > max ) {
...
__forceinline float idMath::ClampFloat( float min, float max, float value ) {
	if ( value < min ) {
		return min;
	}
	if ( value > max ) {
...
__forceinline float idMath::AngleNormalize360( float angle ) {
...
__forceinline float idMath::AngleNormalize180( float angle ) {
...
__forceinline float idMath::AngleDelta( float angle1, float angle2 ) {
	return AngleNormalize180( angle1 - angle2 );
}

__forceinline int idMath::FloatHash( const float *array, const int numFloats ) {
	int i, hash = 0;
	const int *ptr;
